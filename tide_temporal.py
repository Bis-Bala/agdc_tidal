...
@click.option('--debug', 'debug', default=False, type=str, help='Build in debug mode to get details of tide height within time range')
# @ui.parsed_search_expressions
# @ui.pass_index(app_name='agdc-tidal-analysis-app')

def main(epoch, lon_range, lat_range, year_range, tide_post, per, season, debug):
    ...
    td_info = MyTide(dc, lon_range, lat_range, products, epoch, year_range, tide_post, per, season, debug)
...
class MyTide():
    def __init__(self, dc, lon_range, lat_range, products, epoch, year_range, tide_post, per, season, debug):
        self.dc = dc
    ...
    def build_my_dataset(self, acq_min, acq_max):
        ...
        for i, st in enumerate(self.products):
            ...
            if st == 'ls7_nbar_albers' and acq_max > datetime.strptime(dt7, "%Y-%m-%d").date() and  \
                ...
                continue
            elif st == 'ls7_nbar_albers' and acq_max > datetime.strptime(dt7, "%Y-%m-%d").date() and \
                  acq_min < datetime.strptime(dt7, "%Y-%m-%d").date():
    ...
    def tidal_task(self, acq_min, acq_max):
        ...
        print ("calculation for medoid finished for this epoch", str(datetime.now().time()))      
