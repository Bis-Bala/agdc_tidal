...
class MyTide():
    ...
    def get_epochs(self):
        for dt in rrule(YEARLY, interval=self.epoch, dtstart=self.start_epoch, until=self.end_epoch):
            ...
            if dt.date() >= self.end_epoch:
               print ("CALCULATION finished and data available in MY_GEO and MY_MEDOID dictionary ")
               return 
            acq_min = dt.date()
    ...
    def tidal_task(self, acq_min, acq_max):
        ...
        print ("creating MEDOID for lower range " + str(datetime.now().time()))      
        med_low = statistics.combined_var_reduction(ds_low, statistics.nanmedoid)         
        print ("creating MEDOID for higher range " + str(datetime.now().time()))      
        med_high = statistics.combined_var_reduction(ds_high, statistics.nanmedoid)         
        key = ''
        if self.season.upper() != 'DUMMY':
            key = "MEDOID_" + str(acq_min) + "_" + str(acq_max) + "_" + self.season.upper() + "_LOW"
        else:
            key = "MEDOID_" + str(acq_min) + "_" + str(acq_max) + "_LOW"
        MY_MEDOID[key] = copy.deepcopy(med_low)
        key = ''
        if self.season.upper() != 'DUMMY':
            key = "MEDOID_" + str(acq_min) + "_" + str(acq_max) + "_" + self.season.upper() + "_HIGH"
        else:
            key = "MEDOID_" + str(acq_min) + "_" + str(acq_max) + "_HIGH" 
        MY_MEDOID[key] = copy.deepcopy(med_high)
        print ("creating GEOMEDIAN for lower range " + str(datetime.now().time()))      
        med_low = statistics.combined_var_reduction(ds_low, hd.fast.nangeomedian)         
        print ("creating GEOMEDIAN for higher range " + str(datetime.now().time()))      
        med_high = statistics.combined_var_reduction(ds_high, hd.fast.nangeomedian)         
        key = ''
        if self.season.upper() != 'DUMMY':
        ...
        if (acq_max == self.end_epoch):
            print (" calculation finished and data is available in MY_GEO and MY_MEDOID dictionaries ", 
...
if __name__ == '__main__':
    ...
    extracts epoch/seasonal data and calculates medoid and geomedian for six bands and output to a dictionary MY_DATA.
